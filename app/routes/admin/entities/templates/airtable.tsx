...
import airtableService from "~/utils/integrations/airtableService";
...
export let loader: LoaderFunction = async () => {
  const data: LoaderData = {
    apiKeyAlreadySet: (process.env.AIRTABLE_API_KEY?.toString() ?? "").length > 0,
  };
...
export const action: ActionFunction = async ({ request }) => {
  ...
  if (action === "preview") {
    try {
      ...
      const apiKey = process.env.AIRTABLE_API_KEY?.toString() ?? form.get("apiKey")?.toString() ?? "";
      const base = await airtableService.getBase(baseId, { apiKey });
      if (!base) {
        return badRequest({ error: "No Airtable bases found" });
      }
...
export default function AdminEntityTemplatesManual() {
  ...
  // const [apiKey, setApiKey] = useState<string>(data.apiKeyAlreadySet ? "************" : "");
    ...
    >
      <div className="md:border-t md:border-gray-200 md:py-2">
        {actionData?.error ? (
          ...
          <>
            <UnderConstruction title="AIRTABLE: IMPORT BASE TO SAASROCK ENTITIES" />
            {/* <Form method="post">
              ...
              <div className="space-y-3">
                <div className="grid grid-cols-2 gap-2">
                  <InputText
                    ...
                    hint={
                      <div>
                        <a className="underline hover:text-blue-500" href="https://airtable.com/account" target="_blank" rel="noreferrer">
                          Get API Key
                        </a>
                      </div>
                    }
                    disabled={data.apiKeyAlreadySet}
                    value={apiKey}
                    setValue={setApiKey}
                    required
                  />
                  <InputText name="baseId" title="Base ID" value={baseId} setValue={setBaseId} required placeholder="tblL..." />
                </div>
