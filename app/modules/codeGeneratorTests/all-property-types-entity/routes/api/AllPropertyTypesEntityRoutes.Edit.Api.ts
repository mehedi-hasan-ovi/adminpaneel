...
export namespace AllPropertyTypesEntityRoutesEditApi {
  ...
  export let loader: LoaderFunction = async ({ request, params }) => {
    ...
    const item = await AllPropertyTypesEntityService.get(params.id!, {
      tenantId,
      userId,
    });
    if (!item) {
    ...
    const data: LoaderData = {
      ...
      item,
      permissions,
      workflowStates: await getWorkflowStatesByEntity({ name: "allPropertyTypesEntity" }),
  ...
  export const action: ActionFunction = async ({ request, params }) => {
    ...
    const entity = await getEntityByName({ tenantId, name: "allPropertyTypesEntity" });
    ...
    if (action === "edit") {
      try {
        ...
        await AllPropertyTypesEntityService.update(
          params.id!,
      ...
      try {
        await AllPropertyTypesEntityService.del(params.id!, {
          tenantId,
          userId,
        });
        if (item.createdByUser) {
