...
export default function KbFeaturedArticles({ kb, items }: { kb: KnowledgeBaseDto; items: KbArticleDto[] }) {
  return (
    <div className="space-y-4">
      ...
      <div className={clsx("grid gap-2", items.length === 1 ? "grid-cols-1" : "grid-cols-2")}>
        {items.map((item) => {
          return (
            <Link
              to={item.href}
              key={item.id}
              className="rounded-md border border-gray-200 bg-white px-3 py-4 shadow-sm hover:border-slate-400 hover:bg-slate-50"
            >
              <div className="flex items-center justify-between space-x-2">
                <div className="flex-grow font-medium text-gray-700">{item.title}</div>
