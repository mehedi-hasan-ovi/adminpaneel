              ...
              <label
                ...
                className={clsx(
                  clsx(
                    ...
                    isChecked(item.value) ? "border-gray-300 ring-1 ring-gray-300" : "",
                    "relative flex rounded-lg border p-3 shadow-sm focus:outline-none",
                    disabled || item.disabled ? "cursor-not-allowed bg-gray-100" : "cursor-pointer bg-white"
                  )
                )}
              >
                <input
                  ...
                  <div className="flex w-full justify-between truncate">
                    <div className="truncate">
                      {["nameAndValue", "name"].includes(displayType) ? (
                        <div className={clsx("block w-full truncate text-sm", !disabled ? "font-medium text-gray-600" : "text-gray-600")}>{item.name}</div>
                      ) : (
                        <div className={clsx("block w-full truncate text-sm", !disabled ? "font-medium text-gray-600" : "text-gray-600")}>{item.name}</div>
                      )}
                    </div>

                    <CheckEmptyCircle className={clsx("flex-shrink-0", !isChecked(item.value) && "invisible", "h-5 w-5 text-teal-600")} aria-hidden="true" />
                    <span
                      className={clsx(
                        isChecked(item.value) ? "border" : "border-2",
                        isChecked(item.value) ? "border-gray-300" : "border-transparent",
