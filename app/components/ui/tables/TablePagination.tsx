...
export default function TablePagination({ page, pageSize, totalItems, totalPages }: Props) {
  ...
  function onSetState(state: { page: number; pageSize: number | undefined }) {
    const currentPageSizeParam = searchParams.get("pageSize")?.toString();
    const pageSizeParam = currentPageSizeParam ?? Constants.DEFAULT_PAGE_SIZE.toString();
    ...
    >
      ...
      <div className="flex items-center space-x-2">
        {/* <select
          ...
          name="pageSize"
          className={clsx(
            "hidden items-center justify-center rounded border border-gray-200 bg-white text-xs focus:border-accent-500 focus:ring-accent-500 sm:inline-flex",
            page === 1 || loading ? "cursor-not-allowed opacity-90" : "hover:border-gray-300 hover:bg-gray-50"
          )}
          onChange={(e) => onSetState({ ...state, page: 1, pageSize: Number(e.target.value) })}
        ...
        <div className="inline-flex items-center justify-center gap-2">
          <button
            ...
            onClick={() => onSetState({ ...state, page: page - 1 })}
            className={clsx(
              "inline-flex h-8 w-8 items-center justify-center rounded focus:outline-none focus:ring-2 focus:ring-gray-500",
              page === 1 || loading ? "cursor-not-allowed opacity-90" : "hover:border-gray-100"
            )}
          >
          ...
          {showPageNumberModal ? (
            <input
              ...
              value={state.page}
              onChange={(e) => onSetState({ ...state, page: parseInt(e.target.value) })}
            ...
            <select
              ...
              name="page"
              className={clsx(
                "inline-flex items-center justify-center rounded border border-gray-200 bg-gray-50 text-xs hover:border-gray-300 hover:bg-gray-100 focus:border-accent-500 focus:ring-accent-500"
              )}
              onChange={(e) => onSetState({ ...state, page: Number(e.target.value) })}
              value={state.page}
            >
              {[...Array(totalPages)].map((_, idx) => {
                return (
                  <option key={idx} value={idx + 1} className="lowercase">
                    {idx + 1}
                  </option>
                );
              })}
          ...
          <button
            ...
            className={clsx(
              "inline-flex h-8 w-8 items-center justify-center rounded focus:outline-none focus:ring-2 focus:ring-gray-500",
              page >= totalPages || loading ? "cursor-not-allowed opacity-90" : "hover:bg-gray-100"
            )}
