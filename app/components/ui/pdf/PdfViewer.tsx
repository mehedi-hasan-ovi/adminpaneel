...
export default function PdfViewer({ className, file, onRemoveFile, fileName = "", editing = false, canDownload = true }: Props) {
  ...
  return (
    <div id="pdf-viewer" className={clsx(className, "items-center overflow-hidden rounded-md border border-dashed border-gray-300 bg-gray-100 text-gray-600")}>
      {error ? (
        <div className="text-red-500">{error}</div>
      ) : !pdfRef ? (
        <div className="flex h-64 justify-center p-12">
          <div className="flex justify-center p-12 text-sm italic text-gray-500">{t("shared.loading")}...</div>
        </div>
      ) : (
        <>
