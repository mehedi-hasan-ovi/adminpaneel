...
function KanbanColumnCard<T>({ idx, columns, column, items, classNameWidth }: KanbanColumnCardProps<T>) {
  ...
  return (
    ...
    >
      ...
      <div
        className={clsx(
          "h-full overflow-x-hidden border-0 border-dashed border-gray-200 bg-gray-50 px-2 py-3",
          idx === 0 && "border-r-0",
      ...
      >
        <div className="space-y-3">
          ...
          {items.map((item, idx) => {
            return (
              <div key={idx} className="group w-full text-left shadow-sm hover:bg-gray-50">
                {column?.onClickRoute ? (
